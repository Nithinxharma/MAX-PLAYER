...
class DeclarativeCineHubProvider(
    ...
    override suspend fun loadDetails(url: String): CineHubMediaDetails? = withContext(Dispatchers.IO) {
        try {
            ...
            if (isTv) {
                val details = CineOnlineScraper.fetchTvShowDetails(tmdbId) ?: return@withContext null
                val episodes = mutableListOf<CineHubEpisode>()
                details.seasons.forEach { season ->
                    if (season.season_number > 0) {
                        val eps = CineOnlineScraper.fetchTvShowEpisodes(null, tmdbId, season.season_number, details.name)
                        eps.forEach { ep ->
                            episodes.add(CineHubEpisode(
                                id = "${url}_S${ep.season}E${ep.episode}",
                                name = ep.title,
                                season = ep.season,
                                episode = ep.episode,
                                data = "tv:${tmdbId}:${ep.season}:${ep.episode}",
                ...
                CineHubMediaDetails(
                    id = url, title = details.name ?: "", url = url, providerId = id, providerName = name,
                    ...
                    year = details.first_air_date?.take(4)?.toIntOrNull(), rating = details.vote_average,
                    genres = emptyList(), cast = emptyList(),
