package xyz.mpv.rex.cinetv.data
...
object JioTvRepo {
    ...
    fun readM3uText(context: Context): String {
        ...
        return try {
            if (file.exists()) file.readText() else context.assets.open("in.m3u").bufferedReader().readText()
        } catch (e: Exception) { "" }
    }
    ...
    fun loadM3uFallback(context: Context): List<M3uEntry> {
        ...
        try {
            val file = getM3uFile(context)
            val lines = if (file.exists()) file.bufferedReader().readLines() else try { context.assets.open("in.m3u").bufferedReader().readLines() } catch (e: Exception) { emptyList() }
            var currentName = ""
