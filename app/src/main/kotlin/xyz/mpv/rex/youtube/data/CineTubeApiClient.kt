            ...
            try {
                client.newCall(request).execute().use { response ->
                    if (response.isSuccessful) {
                        ...
                        if (data.isNotEmpty()) {
                            android.util.Log.d("CineTube", "[CineTube] Loaded ${data.size} videos from $baseUrl")
                            return@withContext data
                        }
                    }
                }
            } catch (e: Exception) {
                android.util.Log.w("CineTube", "[CineTube] Node $baseUrl failed. Shifting...")
            }
        }
            ...
            try {
                client.newCall(request).execute().use { response ->
                    if (response.isSuccessful) {
                        val responseBody = response.body?.string() ?: return@use
                        val data = jsonParser.decodeFromString<List<YoutubeVideo>>(responseBody)
                        if (data.isNotEmpty()) android.util.Log.d("CineTube", "[CineTube] Loaded ${data.size} videos from $baseUrl")
                            return@withContext data
                    }
                }
            } catch (e: Exception) {
